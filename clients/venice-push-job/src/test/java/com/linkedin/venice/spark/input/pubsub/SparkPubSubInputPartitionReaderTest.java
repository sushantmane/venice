...
public class SparkPubSubInputPartitionReaderTest {
  ...
  @Test
  public void testCloseMethodScenarios() throws IOException {
    // Case 1: Normal close operation
    SparkPubSubInputPartitionReader reader1 = createReaderWithMockIterator();
    reader1.close(); // Should not throw exception

    // Case 2: Multiple close calls should be safe
    SparkPubSubInputPartitionReader reader2 = createReaderWithMockIterator();
    reader2.close();
    reader2.close(); // Second close should be safe

    // Case 3: Close with different reader instances
    SparkPubSubInputPartitionReader reader3 = createReaderWithMockIterator();
    SparkPubSubInputPartitionReader reader4 = createReaderWithMockIterator();
    reader3.close();
    reader4.close();
    // Both should close without issues
  }
