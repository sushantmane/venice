...
public class RocksDBStoragePartition extends AbstractStoragePartition {
  ...
  protected final String versionedStoreName;
  ...
  protected RocksDBStoragePartition(
    ...
    this.versionedStoreName = storagePartitionConfig.getStoreName();
    this.storeName = Version.parseStoreFromVersionTopic(versionedStoreName);
    ...
    this.fullPathForPartitionDB = RocksDBUtils.composePartitionDbDir(dbDir, versionedStoreName, partitionId);
    ...
    this.fullPathForTempSSTFileDir = RocksDBUtils.composeTempSSTFileDir(dbDir, versionedStoreName, partitionId);
    this.fullPathForTempSnapshotFileDir =
        blobTransferEnabled ? RocksDBUtils.composeSnapshotDir(dbDir, versionedStoreName, partitionId) : null;
  ...
  public synchronized void drop() {
    close();
    try {
      Options storeOptions = getStoreOptions(new StoragePartitionConfig(versionedStoreName, partitionId), false);
  ...
  public synchronized void close() {
    ...
    LOGGER.info("RocksDB close for replica: {} took {} ms.", replicaId, LatencyUtils.getElapsedTimeInMs(startTimeInMs));
  }
  ...
  public synchronized void reopen() {
    ...
    try {
      ...
      rocksDB.close();
      LOGGER
          .info("RocksDB close for replica: {} took {} ms.", replicaId, LatencyUtils.getElapsedTimeInMs(startTimeInMs));
  ...
  private void registerDBStats() {
    if (rocksDBMemoryStats != null) {
      rocksDBMemoryStats.registerPartition(RocksDBUtils.getPartitionDbName(versionedStoreName, partitionId), this);
    }
  }

  private void deRegisterDBStats() {
    if (rocksDBMemoryStats != null) {
      rocksDBMemoryStats.deregisterPartition(RocksDBUtils.getPartitionDbName(versionedStoreName, partitionId));
    }
  }
