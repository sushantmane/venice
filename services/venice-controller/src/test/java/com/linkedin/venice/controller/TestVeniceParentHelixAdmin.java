...
public class TestVeniceParentHelixAdmin extends AbstractTestVeniceParentHelixAdmin {
  ...
  public void testAddStore() {
    doReturn(CompletableFuture.completedFuture(new SimplePubsubProduceResultImpl(topicName, partitionId, 1, -1, -1)))
        .when(veniceWriter)
        .put(any(), any(), anyInt());
  ...
  public void testCreateStoreForMultiCluster() {
    ...
    for (String cluster: configMap.keySet()) {
      ...
      when(veniceWriter.put(any(), any(), anyInt())).then(invocation -> {
        ...
        doReturn(new SimplePubsubProduceResultImpl(adminTopic, partitionId, 1, -1, -1)).when(future).get();
        return future;
      });
