 ...
 * For metadata updateAsync generated by hybrid ingestion, it will sync and persist the updateAsync to the RocksDB metadata partition
...
public class MainIngestionStorageMetadataService extends AbstractVeniceService implements StorageMetadataService {
  ...
  public void computeStoreVersionState(String topicName, Function<StoreVersionState, StoreVersionState> mapFunction)
    ...
    topicStoreVersionStateMap.compute(topicName, (key, previousStoreVersionState) -> {
      ...
      // Sync updateAsync with metadata partition opened by ingestion process.
  ...
  /**
   * putOffsetRecord will only putAsync OffsetRecord into in-memory state, without persisting into metadata RocksDB partition.
   */
  ...
  /**
   * putStoreVersionState will only putAsync StoreVersionState into in-memory state, without persisting into metadata RocksDB partition.
   */
  ...
  /**
   * MetadataUpdateWorker is a Runnable class that pushes local metadata updateAsync on the FIFO basis. It will retry updates
  ...
  class MetadataUpdateWorker implements Runnable, Closeable {
    ...
    public void run() {
      while (isRunning.get()) {
        try {
          /**
           * Here it leaves room for future optimization: if the metadata updateAsync volumes is large in classical Venice
           * and this sequential updateAsync approach is lagging, we can transform it into batch updateAsync.
           */
          ...
          if (metadataUpdateQueue.size() > 0) {
            LOGGER.info("Number of remaining metadata updateAsync requests in queue: {}", metadataUpdateQueue.size());
          }
