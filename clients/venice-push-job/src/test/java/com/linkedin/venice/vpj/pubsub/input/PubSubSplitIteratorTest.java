...
public class PubSubSplitIteratorTest {
  ...
  public void testCloseMethodScenarios() {
    ...
    // Case 3: Close after consuming records
    List<DefaultPubSubMessage> messages = createMockDataMessages(1, 0L);
    PubSubConsumerAdapter mockConsumer3 = mock(PubSubConsumerAdapter.class);
    setupMockConsumerPolling(mockConsumer3, messages);
    when(mockConsumer3.positionDifference(any(), any(), any())).thenReturn(10L, 9L);

    PubSubSplitIterator iterator3 = new PubSubSplitIterator(mockConsumer3, testSplit, false);
    try {
      iterator3.next();
    } catch (Exception e) {
      // Ignore for test
