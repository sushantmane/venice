...
public class TopicManagerStatsTest {
  ...
  public void testRecordLatencyInteractions() {
    ...
    TopicManagerStats.recordLatency(null, TopicManagerStats.OCCURRENCE_LATENCY_SENSOR_TYPE.CONTAINS_TOPIC, 100);
    ...
    EnumMap<TopicManagerStats.OCCURRENCE_LATENCY_SENSOR_TYPE, Sensor> sensorsByTypes =
        new EnumMap<>(TopicManagerStats.OCCURRENCE_LATENCY_SENSOR_TYPE.class);
    Sensor sensorMock = mock(Sensor.class);
    sensorsByTypes.put(TopicManagerStats.OCCURRENCE_LATENCY_SENSOR_TYPE.CONTAINS_TOPIC, sensorMock);
    when(topicManagerStatsMock.getSensorsByTypes()).thenReturn(sensorsByTypes);
    TopicManagerStats
        .recordLatency(topicManagerStatsMock, TopicManagerStats.OCCURRENCE_LATENCY_SENSOR_TYPE.CONTAINS_TOPIC, 100);
    verify(sensorMock).record(100);

    // Test: when sensor is null, no exception is thrown.
    sensorsByTypes.put(TopicManagerStats.OCCURRENCE_LATENCY_SENSOR_TYPE.CONTAINS_TOPIC, null);
    TopicManagerStats
        .recordLatency(topicManagerStatsMock, TopicManagerStats.OCCURRENCE_LATENCY_SENSOR_TYPE.CONTAINS_TOPIC, 100);
  }
  ...
  public void testRecordLatency() {
    ...
    EnumMap<TopicManagerStats.OCCURRENCE_LATENCY_SENSOR_TYPE, Sensor> sensorsByTypes =
        topicManagerStats.getSensorsByTypes();
    assertNotNull(sensorsByTypes);
    assertEquals(sensorsByTypes.size(), TopicManagerStats.OCCURRENCE_LATENCY_SENSOR_TYPE.values().length);
