  ...
  public void testSkipMessageEndToEnd() throws ExecutionException, InterruptedException, IOException {
    try (ZkServerWrapper zkServer = ServiceFactory.getZkServer();
      ...
      try (
        ...
        long badOffset =
            writer.putSync(new byte[0], message, AdminOperationSerializer.LATEST_SCHEMA_ID_FOR_ADMIN_OPERATION)
                .get()
                .getOffset();

        byte[] goodMessage = getStoreCreationMessage(clusterName, storeName, owner, keySchema, valueSchema, 2);
        writer.putAsync(new byte[0], goodMessage, AdminOperationSerializer.LATEST_SCHEMA_ID_FOR_ADMIN_OPERATION);
