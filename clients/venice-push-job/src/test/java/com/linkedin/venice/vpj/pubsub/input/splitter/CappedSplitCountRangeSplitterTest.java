...
public class CappedSplitCountRangeSplitterTest {
  private static final PubSubTopicRepository TOPIC_REPOSITORY = new PubSubTopicRepository();

  private CappedSplitCountRangeSplitter splitter;
  ...
  public void setUp() {
    ...
    splitter = new CappedSplitCountRangeSplitter();
  }
  ...
  public void testEdgeCaseSplitScenarios() {
    ...
    assertEquals(splits.get(0).getRangeIndex(), 0);
  ...
  public void testZeroAndNegativeRecordScenarios() {
    ...
    // Case 2: Negative records - should return empty list
    reset(topicManager);
    setupTopicManagerMocks(startPos, endPos, -10);
  ...
  private void assertFiveEvenSplits(
      List<PubSubPartitionRangeSplit> splits,
      long startOffset,
      long step,
  ...
  public void testRemainderDistribution(long totalRecords, int maxSplits, long[] expectedCounts) {
    ...
    long totalSplitRecords = splits.stream().mapToLong(PubSubPartitionRangeSplit::getNumberOfRecords).sum();
  ...
  public void testSplitPositionProgression() {
    ...
    for (PubSubPartitionRangeSplit split: splits) {
  ...
  public void testLargeScaleSplitting() {
    ...
    for (int i = 1; i <= 100; i++) {
      positions[i] = ApacheKafkaOffsetPosition.of(i * 10000);
  ...
  private SplitRequest createSplitRequest(int maxSplits) {
    return new SplitRequest.Builder().pubSubTopicPartition(partition)
        ...
        .splitType(SplitType.CAPPED_SPLIT_COUNT)
        .maxSplits(maxSplits)
        .build();
  }
  ...
  private void verifyEvenSplits(List<PubSubPartitionRangeSplit> splits, long expectedSize) {
    for (PubSubPartitionRangeSplit split: splits) {
  ...
  private void verifyRangeIndexes(List<PubSubPartitionRangeSplit> splits) {
    for (int i = 0; i < splits.size(); i++) {
      assertEquals(
          splits.get(i).getRangeIndex(),
          i,
          String.format("Split at index %d should have rangeIndex %d but has %d", i, i, splits.get(i).getRangeIndex()));
    }
  }
